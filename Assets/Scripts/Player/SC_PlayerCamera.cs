...
public class SC_PlayerCamera : MonoBehaviour
{
    ...
    [Tooltip("フィールド中心"), SerializeField] private Transform fieldCenter;
    ...
    [Tooltip("フィールド中心へ向きを合わせる速さ"), SerializeField] private float yawFollowSpeed = 3f;
    ...
    // 通常カメラの方位を、フィールド中心→プレイヤーの放射方向に合わせる。
    private void UpdateLookAtCenterYaw()
    {
        float dt = Time.deltaTime;
        if (dt <= 0f) return;

        Vector3 center = fieldCenter != null ? fieldCenter.position : Vector3.zero;
        Vector3 toCenter = Vector3.ProjectOnPlane(center - playerTransform.position, Vector3.up);

        // プレイヤーが中心とほぼ同位置なら向きを維持（ゼロ割・ジッタ防止）
        if (toCenter.sqrMagnitude <= ZERO_THRESHOLD) return;
